...
export default function AccountantBankStatementsPage() {
  ...
  const load = () => {
    ...
    fetch('/api/bank-accounts').then(r=>r.json()).then(d=>{
      ...
      setAccounts(accs);
      if (accs.length && !selAccount) setSelAccount(accs[0]._id);
    });
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      date: form.date, description: form.description, remark: form.remark,
  ...
  // API returns asc. Reverse for newest-first display. Recalculate displayBalance top-to-bottom.
  ...
  const filtered = [...statements].reverse().map((s, i, arr) => {
    const newerCredit = arr.slice(0,i).reduce((sum,x) => sum+(x.creditAmount||0), 0);
    const newerDebit  = arr.slice(0,i).reduce((sum,x) => sum+(x.debitAmount||0),  0);
  ...
  const exportExcel = async () => {
    ...
    const rows = filtered.map((s,i) => ({
      ...
      'Credit (Rs.)': s.creditAmount||0, 'Debit (Rs.)': s.debitAmount||0,
      'Balance (Rs.)': s.displayBalance||0,
    }));
            ...
            <form onSubmit={submit} className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <input required value={form.description}
                  ...
                  placeholder="e.g. Cash withdrawal, Bank charges..." />
              </div>
              <div>
                ...
                <div className="flex items-start justify-between mb-1">
                  <div className="flex-1 min-w-0 mr-3">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {new Date(s.date).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})} · {s.bankAccount?.bankName}
                    </p>
                  </div>
              ...
              {filtered.length > 0 && (
                <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                  <tr>
                    ...
                    <td className={`px-4 py-3 font-extrabold whitespace-nowrap ${lastBalance >= 0 ? 'text-blue-700' : 'text-orange-600'}`}>
                      {fmt(lastBalance)}
                    </td>
                  </tr>
                </tfoot>
