...
export default function AdminBankStatementsPage() {
  ...
  // API returns asc (oldest first). Reverse for newest-first display.
  // Recalculate running balance top-to-bottom so each row shows balance AFTER that transaction.
  ...
  // Assign correct running balances for newest-first display
  // Start from closing balance and work backwards
  const filtered = [...statements].reverse().map((s, i, arr) => {
    // running balance after this entry = closingBalance minus sum of all entries newer than this
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((s,i)=>(
                <tr key={s._id} className="hover:bg-gray-50/80 transition">
                  ...
                  <td className={`px-4 py-3 font-bold whitespace-nowrap ${(s.displayBalance||0)>=0?'text-blue-700':'text-orange-600'}`}>{fmt(s.displayBalance||0)}</td>
                </tr>
              ))}
            ...
            <div key={s._id} className="px-4 py-3.5">
              <div className="flex items-start justify-between mb-1">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs text-gray-500">Bal: {fmt(s.displayBalance||0)}</p>
                </div>
              </div>
