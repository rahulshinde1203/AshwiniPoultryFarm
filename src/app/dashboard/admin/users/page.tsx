...
async function downloadUserPDF(user: any) {
  ...
  doc.text('Ashwini Poultry Farm', 14, 14);
  doc.setFontSize(10);
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.text('Ashwini Poultry Farm — Confidential Staff Record', 14, doc.internal.pageSize.getHeight() - 8);
...
//         </div>
//         {file && (
//           <button type="button" onClick={e => { e.stopPropagation(); onClear(); if (ref.current) ref.current.value = ''; }}
...
//             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//               <FileField
//                 field="aadhaarPhoto" label="Aadhaar Card Photo"
//                 file={form.aadhaarPhoto} error={errors.aadhaarPhoto}
//                 onChange={onFileChange('aadhaarPhoto')}
//                 onClear={() => onFileClear('aadhaarPhoto')}
//               />
//               <FileField
//                 field="panPhoto" label="PAN Card Photo"
//                 file={form.panPhoto} error={errors.panPhoto}
//                 onChange={onFileChange('panPhoto')}
//                 onClear={() => onFileClear('panPhoto')}
//               />
//             </div>
...
// /* ─────────────────────────────────────────────────────────
//    DOWNLOAD HELPER — image or PDF file from base64
// ───────────────────────────────────────────────────────── */
// function downloadDoc(base64: string, filename: string) {
//   const a = document.createElement('a');
//   a.href = base64;
...
//   doc.text('PERSONAL INFORMATION', 14, y); y += 2;
//   doc.setDrawColor(249, 115, 22); doc.setLineWidth(0.5); doc.line(14, y, W - 14, y); y += 5;
...
//   doc.text('IDENTITY DOCUMENTS', 14, y); y += 2;
//   doc.setDrawColor(37, 99, 235); doc.setLineWidth(0.5); doc.line(14, y, W - 14, y); y += 5;
...
//   // ── Embed Aadhaar photo if image ──
//   if (user.aadhaarPhoto && user.aadhaarPhoto !== 'placeholder' && !user.aadhaarPhoto.startsWith('data:application/pdf')) {
//     try {
//       doc.setFontSize(8); doc.setTextColor(100,116,139); doc.setFont('helvetica','bold');
//       doc.text('Aadhaar Card Photo:', 14, y + 4); y += 6;
//       doc.addImage(user.aadhaarPhoto, 14, y, 80, 50);
//       y += 56;
//     } catch { /* skip if image fails */ }
//   } else if (user.aadhaarPhoto && user.aadhaarPhoto !== 'placeholder') {
//     doc.setFontSize(8); doc.setTextColor(100,116,139);
//     doc.text('Aadhaar Photo: PDF file (download separately)', 14, y + 4); y += 10;
//   }

//   // ── Embed PAN photo if image ──
//   if (user.panPhoto && user.panPhoto !== 'placeholder' && !user.panPhoto.startsWith('data:application/pdf')) {
//     try {
//       doc.setFontSize(8); doc.setTextColor(100,116,139); doc.setFont('helvetica','bold');
//       doc.text('PAN Card Photo:', 14, y + 4); y += 6;
//       doc.addImage(user.panPhoto, 14, y, 80, 50);
//       y += 56;
//     } catch { /* skip if image fails */ }
//   } else if (user.panPhoto && user.panPhoto !== 'placeholder') {
//     doc.setFontSize(8); doc.setTextColor(100,116,139);
//     doc.text('PAN Photo: PDF file (download separately)', 14, y + 4); y += 10;
//   }
...
//   doc.text('BANK DETAILS', 14, y); y += 2;
//   doc.setDrawColor(5, 150, 105); doc.setLineWidth(0.5); doc.line(14, y, W - 14, y); y += 5;
...
//   const handleDownloadPDF = async () => {
//     setDownloading(true);
//     try { await downloadUserPDF(user); } catch { toast.error('PDF generation failed'); }
//     setDownloading(false);
//   };
...
//                       </div>
//                     ) : (
//                       <img src={user.aadhaarPhoto} alt="Aadhaar" className="w-full max-h-52 object-contain rounded-xl border border-gray-200 bg-gray-50 cursor-pointer"
//                         onClick={() => downloadDoc(user.aadhaarPhoto, 'aadhaar_card.jpg')} />
//                     )
//                   ) : <p className="text-sm text-gray-400 italic">No photo uploaded</p>}
//                 </div>
...
//                       </div>
//                     ) : (
//                       <img src={user.panPhoto} alt="PAN" className="w-full max-h-52 object-contain rounded-xl border border-gray-200 bg-gray-50 cursor-pointer"
//                         onClick={() => downloadDoc(user.panPhoto, 'pan_card.jpg')} />
//                     )
//                   ) : <p className="text-sm text-gray-400 italic">No photo uploaded</p>}
//                 </div>
...
//       if (!file) return;
//       if (!['image/jpeg','image/jpg','image/png','application/pdf'].includes(file.type)) {
//         toast.error('Only JPG, JPEG, PNG, PDF allowed'); return;
//       }
...
//       });
//       const d = await res.json();
//       if (res.ok) { toast.success('User created! 🎉'); setMode('list'); setForm(EMPTY_FORM); load(); }
//       else setErrors({ general: d.error || 'Something went wrong' });
//     } catch { setErrors({ general: 'Upload failed. Try again.' }); }
//     setLoading(false);
//   };
...
//       });
//       const d = await res.json();
//       if (res.ok) { toast.success('User updated!'); setMode('list'); load(); }
//       else setErrors({ general: d.error || 'Update failed' });
//     } catch { setErrors({ general: 'Something went wrong' }); }
//     setLoading(false);
//   };

//   const submitPwd = async () => {
//     if (!PWD_RE.test(newPwd)) { setNewPwdErr('Must be 8-16 chars with uppercase, lowercase, number & special char'); return; }
//     if (newPwd !== confirmNewPwd) { setNewPwdErr('Passwords do not match'); return; }
//     setNewPwdErr(''); setLoading(true);
//     const res = await fetch(`/api/users/${editUser._id}`, {
//       method: 'PUT', headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify({ password: newPwd }),
//     });
//     setLoading(false);
//     if (res.ok) { toast.success('Password changed!'); setMode('list'); }
//     else { const d = await res.json(); setNewPwdErr(d.error || 'Failed'); }
//   };

//   const openEdit = (u: any) => {
//     setEditUser(u);
//     setForm({ ...EMPTY_FORM, name: u.name, email: u.email, mobile: u.mobile || '',
//       aadhaarNumber: u.aadhaarNumber || '', aadhaarPhoto: null, panNumber: u.panNumber || '',
//       panPhoto: null, bankName: u.bankName || '', accountNumber: u.accountNumber || '',
//       ifscCode: u.ifscCode || '', address: u.address || '', password: '', confirmPassword: '', role: u.role });
//     setErrors({}); setMode('edit');
//   };

//   const openPwd = (u: any) => {
//     setEditUser(u); setNewPwd(''); setConfirmNewPwd(''); setNewPwdErr(''); setMode('password');
//   };
...
//     toast.success(isActive ? 'User disabled' : 'User enabled'); load();
//   };
...
//       showPwd={showPwd} showCPwd={showCPwd}
//       onBack={() => setMode('list')}
//       onChange={handleChange}
//       onFileChange={handleFileChange}
//       onFileClear={handleFileClear}
...
//   if (mode === 'edit') return (
//     <UserForm
//       isEdit editUserName={editUser?.name}
//       form={form} errors={errors} loading={loading}
//       showPwd={showPwd} showCPwd={showCPwd}
//       onBack={() => setMode('list')}
//       onChange={handleChange}
...
//               onChange={e => { setNewPwd(e.target.value); setNewPwdErr(''); }}
//               className={`${inp(!!newPwdErr)} pr-10`} placeholder="Enter new password"
//               maxLength={16} autoComplete="new-password" />
//             <button type="button" onClick={() => setShowPwd(!showPwd)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">{showPwd ? '🙈' : '👁️'}</button>
//           </div>
//           {newPwd && (
//             <div className="mt-2">
//               <div className="flex gap-1 mb-1">{Array(5).fill(0).map((_, i) => <div key={i} className={`flex-1 h-1.5 rounded-full ${i < pwS ? STRENGTH_COLORS[pwS] : 'bg-gray-200'}`} />)}</div>
//               <p className={`text-xs font-medium ${pwS >= 4 ? 'text-green-600' : 'text-orange-600'}`}>{STRENGTH_LABELS[pwS]}</p>
//             </div>
//           )}
//         </div>
...
//           <label className={lbl}>Confirm New Password *</label>
//           <div className="relative">
//             <input type={showCPwd ? 'text' : 'password'} value={confirmNewPwd}
//               onChange={e => { setConfirmNewPwd(e.target.value); setNewPwdErr(''); }}
//               className={`${inp(!!newPwdErr)} pr-10`} placeholder="Re-enter new password"
//               maxLength={16} autoComplete="new-password" />
//             <button type="button" onClick={() => setShowCPwd(!showCPwd)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">{showCPwd ? '🙈' : '👁️'}</button>
//           </div>
//           {confirmNewPwd && <p className={`text-xs mt-1 ${newPwd === confirmNewPwd ? 'text-green-600' : 'text-red-500'}`}>{newPwd === confirmNewPwd ? '✓ Match' : '✗ Do not match'}</p>}
//         </div>
//         <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
//           <div className="grid grid-cols-2 gap-1">
//             {[
...
//         {newPwdErr && <p className="text-sm text-red-600 font-medium">⚠️ {newPwdErr}</p>}
//         <div className="flex gap-3 pt-1">
//           <button onClick={submitPwd} disabled={loading}
//             className="flex-1 py-3 bg-orange-500 text-white rounded-xl font-bold text-sm hover:bg-orange-600 disabled:opacity-50">
//             {loading ? 'Saving...' : '🔐 Change Password'}
//           </button>
//           <button onClick={() => setMode('list')} className="px-6 py-3 border border-gray-200 rounded-xl text-sm hover:bg-gray-50">Cancel</button>
//         </div>
//       </div>
//     </div>
//   );
...
//             <p className="text-slate-300 text-sm mt-0.5">{users.length} staff accounts</p>
//           </div>
//           <button onClick={() => { setForm(EMPTY_FORM); setErrors({}); setShowPwd(false); setShowCPwd(false); setMode('add'); }}
//             className="flex items-center gap-2 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold text-sm shadow-md transition">
//             <span>+</span> Add User
//           </button>
//         </div>
...
//                   <div className="text-4xl mb-2">👥</div><p className="font-medium">No users found</p>
//                 </td></tr>
//               )}
//             </tbody>
//           </table>
